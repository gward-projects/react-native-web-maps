...
function useAdvancedMarker(props: AdvancedMarkerProps) {
  ...
  const isEmpty = React.Children.toArray(children).every(
    (child) => (child as ReactElement).type === Callout,
  );
  ...
  useEffect(() => {
    ...
    if (onRightClick && clickable) {
      // @ts-ignore
      marker.content?.addEventListener('contextmenu', onRightClick, { signal });
...
function partitionChildren(children: ReactNode): [ReactNode[], ReactNode[]] {
...
const AdvancedMarkerComponent = forwardRef(
  (props: AdvancedMarkerProps, ref: Ref<AdvancedMarkerRef>) => {
    ...
    const [htmlContent, callouts] = partitionChildren(children);
    ...
    return (
      <AdvancedMarkerContext.Provider value={advancedMarkerContextValue}>
        {contentContainer !== null
          ? createPortal(htmlContent, contentContainer)
          : null}
        {callouts}
